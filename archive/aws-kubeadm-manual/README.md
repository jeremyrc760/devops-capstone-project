...
This archived directory contains the standard Kubernetes manifests originally used to deploy the Account API to a self-managed AWS kubeadm cluster. The current deployment uses `deploy/applications/accounts/kustomize/overlays/aws-kubeadm/` instead.
...
```bash
kubectl apply -f archive/aws-kubeadm-manual/namespace.yaml
```
...
```bash
kubectl apply -k archive/aws-kubeadm-manual
```
